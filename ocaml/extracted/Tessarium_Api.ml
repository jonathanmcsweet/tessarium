...
let encode (f : ('key, 'tweak) Tessarium_Feistel.round_fn) (k : 'key)
  (t : 'tweak) (lat : Tessarium_Spec.lat_ns)
  (lon : Tessarium_Spec.lon_ns) : Tessarium_Codec.address=
  Tessarium_Codec.to_address
    (Tessarium_Feistel.encrypt f k t
       (Tessarium_Grid.point_to_cell lat lon))
let decode (f : ('key, 'tweak) Tessarium_Feistel.round_fn) (k : 'key)
  ...
  let i =
    Tessarium_Feistel.decrypt f k t (Tessarium_Codec.from_address a) in
