...
let () =
  expect "kat 1"
    (hex
       (Tessarium_argon2.kdf ~password:"abandon abandon art"
  ...
  expect "kat 3 (salt with spaces)"
    (hex
       (Tessarium_argon2.kdf ~password:"legal winner thank year"
  ...
  (incr checks;
   ...
   | _ ->
       Printf.eprintf "argon2: a 5-byte salt was accepted\n";
       exit 1
   | exception Invalid_argument _ -> ());
  Printf.printf
    "argon2id answers for %d known-answer checks (t=3, m=64MiB, p=1)\n" !checks
