...
let rejects name json = check name (Result.is_error (Ledger.of_json json))
...
let () =
  ...
  List.iter
    (fun key ->
      rejects
        (Printf.sprintf "an entry missing %s" key)
  ...
  rejects "an empty name" (ledger [ with_field "name" (`String "") (entry ()) ]);
  ...
  check "a whole-number corner is still a number"
    (Result.is_ok
       (Ledger.of_json
          (ledger
             [
               entry ~regions:[ with_field "min_lat" (`Int 51) (region ()) ] ();
             ])));
       ...
       (Ledger.of_json
          (ledger
             [
               entry
                 ~regions:
                   [ region ~extra:[ ("polygon", `List [ ring square ]) ] () ]
                 ();
             ])));
  rejects "a polygon that is not a list of rings"
    (ledger
       [
         entry
           ~regions:[ region ~extra:[ ("polygon", `String "square") ] () ]
           ();
       ]);
       ...
       [
         entry
           ~regions:
             [
               region
                 ~extra:[ ("polygon", `List [ `List [ `List [ `Float 0. ] ] ]) ]
                 ();
             ]
           ();
       ]);
