(** Turning a request target into a path we are willing to open.
    ...
    here: {!resolve} wraps [Tessarium_UrlPath.resolve], extracted from F*,
    where two theorems are proved of it. Every segment it accepts is
    non-empty, is neither ["."] nor [".."], and carries no ['/'], ['\\'] or
    NUL; and a target that DECODES to a traversal is refused however it was
    spelled.
    ...
    {!content_type} is not a cosmetic decision -- a wrong type on a script is
    a security question. *)

val resolve : string -> string list option
(** [resolve target] is the path segments to open under the asset root, or
    [None] for a target we refuse. A directory target resolves to
    [index.html], so the app is reachable at ["/"].

    Targets longer than 8 KiB are refused unread. That is a resource limit
    rather than a safety one -- the proved resolver is linear in a list of
    boxed bytes, and this runs on every request before the rate limiter. *)
...
(** The inverse. Walks the list once: written the obvious way, with
    [List.nth] inside [String.init], it is quadratic -- 4 KB of path took
    6.6 ms against 20 us for this -- and it runs on every request.

    Both conversions are public so the test suite can hold this wrapper to
    the proved module's own predicates. *)
