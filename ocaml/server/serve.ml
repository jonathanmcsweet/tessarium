(* The effectful edge: sockets, files, clocks and the session cache.
   ...
   Every decision here was already made by a pure function --
   [Route.of_request], [Http_range.parse], [Url_path.content_type]. What is
   left is IO that cannot be avoided.

   Large files are streamed, never read whole. A planet-scale PMTiles basemap
   is ~100 GB and is served entirely through range requests; loading a
   response body into memory works for every asset in the UI build and then
   falls over on the one file that matters. *)
...
(* A seed phrase is typed into this page. The CSP is what stops a compromised
   dependency posting it somewhere: with connect-src at 'self' there is
   nowhere to post it to. So the default admits no remote origin at all, and
   widening it takes a flag.

   blob: is in worker-src because MapLibre GL builds its tile workers that
...
let content_security_policy cfg =
  ...
  String.concat "; "
    [
      "default-src 'self'";
      (* 'wasm-unsafe-eval' allows WebAssembly.compile and nothing else;
         scripts stay 'self'-only. Both of the worker's wasm modules need it:
         the Argon2id KDF, and the map core that computes every address the
         UI shows. *)
...
(* Returns the status and length alongside the response, so the access log
   reports what was actually sent. A byte count that is always zero is worse
   than none, and so is a status that is always 200.

   [close] ends the connection after this response. Needed in one case: an
   oversized body cannot be drained, so the rest of it would still be on the
   socket when the next request on a keep-alive connection began parsing, and
   that request would read the tail of the last one as its request line. The
...
(* "You already have this." A 304 carries the validator and the freshness
   rule that produced it, and no body -- so no content-length either, since
   there is no representation to describe. `Expert` for the same reason the
   204 below is: cohttp's string-body path would add a length back. *)
...
(* Repeated header lines mean the comma-joined list (RFC 9110 5.3), and an
   intermediary may split one into several. [Http.Header.get] returns only the
   last, so a client whose tag arrived first would be told to download what it
   already has. *)
...
(* Used by tiles and embedded assets alike: gzip goes out as stored to a
   client that accepts it, and inflated for one that does not. Browsers all
   accept it; the exceptions are curl and scripts, which should still work.

   Those are also who write the headers this used to get wrong. Looking for
   the four letters anywhere in the field answered "yes" to `gzip;q=0`, which
   says the opposite, and to `identity, notgzip`. So the field is parsed as
   RFC 9110 12.5.3 describes it: a comma-separated list of codings, each
   optionally weighted, where q=0 means not acceptable and a coding named
   outright beats the `*` wildcard.

   The weight is compared as text rather than parsed as a number. A qvalue is
   0-1 with at most three decimals, so "is it zero" is a spelling question and
   needs no float. *)
let zero_weight q =
...
(* [Eio.Path.kind] answers [`Not_found] for a path that is simply absent, and
   ...
   loop gives ELOOP. Left unhandled, that becomes a connection error -- one
   unauthenticated request could end the connection instead of being answered.

   The client gets the same answer for all of them: there is nothing here to
   send, and which kind of nothing is not a stranger's business. The operator
   gets more. A plain absence returns [`Not_found] silently; anything that
   raises is abnormal and is logged, because "every asset went 404 at once" is
   what running out of file descriptors looks like from outside, and without
   the log line that is indistinguishable from a missing file.
...
(* One tile archive by name. Opened per request and closed with it: the format
   caps the root directory at 16 KB, so this is a header and a few small reads
   against the page cache. A rename cannot disturb an open handle, since the
   fd pins the inode it opened. *)
...
(* One TileJSON document. [query] is the style's ?v= cache-buster, threaded
   into the tile URLs so swapping styles changes them. *)
let tilejson_body cfg ~host ~query ~if_none_match ~min_zoom ~max_zoom ~bounds =
  ...
  (* Small, but these are the two GETs a session makes that would otherwise
     carry no validator, and "everything is revalidated" is worth being true.
     Tagged over the rendered body, which restates the archive headers, so it
     changes exactly when a download changes them. *)
...
(* The source metadata the pmtiles protocol used to read out of the archive
   header, restated as TileJSON.

   The zoom range matters. MapLibre pins canonical tile requests at the
   source's maxzoom and overzooms past it, so a hardcoded 15 over a
   world-at-z6 archive asked for z15 tiles nobody holds and rendered blank at
   street zoom, over data the archive had. The bounds stop it asking about the
   rest of the planet at all.

   Two sources are described here, over the same tile endpoint and the same
   archives, and between them they split the pyramid in two.

   [`Floor] is the map underneath, and its one job is to have no holes: a hole
   draws as an empty tile, an empty tile counts as data, and data replaces the
   coarse tile already on screen. So its depth is measured rather than
   declared -- the deepest zoom the archives cover the whole planet at -- and
   its bounds are the whole planet, because that is what "complete to this
   depth" means. Past that depth MapLibre overzooms instead of asking, and an
   overzoomed tile cannot be missing.

   [`Detail] is what was downloaded: deep and full of holes, because the
   downloader takes any region to any depth. Its bounds are the union of what
   is held, so the map does not ask about a planet nobody fetched, and it
   starts one zoom below where the floor stops rather than at zero -- the
   shallow end is the floor's to draw, out of the same tiles. *)
let serve_tilejson cfg ~basemap_root ~host ~query ~if_none_match ~which =
  ...
  (* Only archives whose data is really on disk count towards a floor -- see
     [Basemap_download.data_is_whole]. A file cut short after its directories
     were written answers every lookup and fails half the reads, which is a
     floor full of holes. *)
  let floor_depth () =
    Basemap_download.floor_depth
      (List.filter_map
         (fun (name, a) ->
           (* The archive is open and its fd pins the inode, but the NAME may
              be gone: the downloader renames over it in this same directory.
              An archive that cannot be sized does not certify a floor, and
              is not a reason to lose the connection. *)
  ...
  | `Floor ->
      (* [max 0]: a depth of -1 means not even the zoom-0 tile is there, and
         a source cannot declare a negative range. It asks for that one tile,
         gets a 204, and draws nothing -- the truth about an empty basemap
         directory. *)
      json ~min_zoom:0 ~max_zoom:(max 0 (floor_depth ())) ~bounds:whole_planet
  | `Detail ->
      (* Headers only, so they come from the stamped cache that already holds
         them rather than from opening every archive again. This document is
         fetched on every style load and after every job transition, and it
         was paying one open per downloaded region for bytes [Tile_set] had
         already read. The floor branch above still opens files: it needs
         [Archive.locate], which a header cannot answer. *)
      let headers =
        List.map
      ...
      let min_zoom, max_zoom, bounds =
        match headers with
        (* Nothing downloaded: an empty range, which is a source that asks
           for nothing. MapLibre skips every tile shallower than [minzoom]
           and never looks deeper than [maxzoom], so this costs no requests
           instead of a viewport of misses on every pan, which is what a
           fresh install used to pay until its first download.

           This number used to have to lie. The coverage query clamped
           itself to it, so an honest depth here dragged that query down to
           the floor's zoom, where the overview answers "present" and the
           offer to download the area on screen never appeared -- exactly
           when it is most needed. The clamp happens on the server now,
           against the archives that really hold detail, so this field has
           one job again. *)
        | [] -> (1, 0, whole_planet)
        | h :: t ->
            let fold f field =
      ...
      in
      (* This is where the cut between the two documents falls. Below it the
         floor is complete, and the tiles it draws are the same bytes this
         source would draw, so asking for them twice buys a duplicate of
         every tile in the viewport and nothing else.
         ...
         deeper than [maxzoom]. Left overlapping instead, an archive holding
         only the world overview would paint its coarsest tiles OVER the
         floor's finer ones, making the map worse inside the region that was
         downloaded. *)
      let min_zoom = max min_zoom (floor_depth () + 1) in
      json ~min_zoom ~max_zoom ~bounds

(* One vector tile. Bytes go out as stored when the client accepts the
   archive's compression, inflated when it does not -- same policy as the
   embedded assets, for the same curl-and-scripts reason.

   A tile nobody holds is a 204, not a 404. Past the edge of what was
   downloaded an empty tile is a normal answer the map draws as nothing,
   whereas an error would be logged as one on every pan. *)
let serve_tile cfg ~basemap_root ~meth ~client_headers ~z ~x ~y =
  let id = Pmtiles.Tile_id.of_zxy ~z ~x ~y in
  let found =
    Eio.Switch.run @@ fun sw ->
    (* Filtered before any file is opened, then opened one at a time and only
       as far as the answer.

       The list is a directory listing -- one file per downloaded region --
       so it grows with every region the user keeps, and opening all of them
       would slow the map down accordingly. [Tile_set.may_hold] reads each
       archive's remembered header and drops the ones whose zoom range or
       bounding box cannot hold this tile, which for a viewport tile is
       nearly all of them. What is left is usually one file, sometimes two:
       the same work the old fixed list of three did. *)
    List.find_map
  ...
  match found with
  | None ->
      (* No validator here, on purpose. A tag looks like the obvious saving,
         since past the edge of a download most of a viewport is this reply,
         but it was measured to buy nothing: Chromium does not store a 204,
         so it never sends the tag back and the 304 never happens, while a
         control on the same origin revalidated a real tile correctly. A 204
         has no body to re-send anyway. It also has no representation, which
         makes an ETag on it a fiction (RFC 9110 8.8.3) and would turn
         `If-None-Match: *` into a 304 where the spec asks for the 204.

         A 204 carries no content-length (RFC 9110) and nothing is written.
  ...
  | Some (bytes, header) ->
      (* Which encoding this client gets, and whether the stored bytes go out
         as they are. [inflate] is deferred: a request that ends in a 304
      ...
      (* Hashed per request rather than derived from the archive, because the
         archive is what changes underneath: a browse-cache download lands
         new tiles at URLs the browser already holds, then renames
         cache.pmtiles into place. A tag over the bytes cannot miss that.

         Over the bytes AS STORED, with the encoding named alongside, so the
         two encodings of one tile get two tags without inflating anything to
         find out. *)
        ...
        let headers =
          Http.Header.of_list
            ...
            :: ("content-length", string_of_int (String.length bytes))
               ...
               style's ?v= carries the swaps. *)
            :: ("cache-control", "no-cache")
            :: ("etag", etag)
            :: (vary @ encoding @ security_headers cfg))
        in
        let response = Http.Response.make ~status:`OK ~headers () in
        (* HEAD gets the headers and no body, or the tile bytes would be read
           as the start of the next response on this connection. *)
...
(* The built UI, compiled into the binary. This is what makes the desktop
   target one file rather than a file plus a directory travelling with it.
   Empty when the UI has not been built, and then everything falls through to
   the directory named by --ui.
...
let serve_embedded cfg ~segments ~meth ~headers =
  ...
  | Some (digest, packed) ->
      ...
      let extra =
        match enc with None -> [] | Some e -> [ ("content-encoding", e) ]
      in
      (* Responses differ by whether the client took the gzip, so a shared
         cache must not hand one client the other's bytes. The two encodings
         are two representations and get two tags. *)
      let vary = [ ("vary", "accept-encoding") ] in
      ...
      then Some (not_modified cfg ~etag ~cache_control ~vary)
        ...
        (* Decompressed only once we know it is being sent. This is the built
           UI, and a reload that ends in a 304 should not inflate it just to
           say so. *)
        let body =
...
(* A window onto an open file, so a range response never lands in memory.
   Eio's File.pread reads from an explicit offset, so there is no shared seek
   position. *)
...
(* Serving a file is `Expert` rather than `Response` because the body has to
   be written while the file is still open. cohttp-eio runs the response
   closure after the callback returns, so a `with_open_in` around a `Response`
   closes the file before a byte is written.

   The file is OPENED BEFORE the response is built, and the size and the ETag
   come from that open handle, not from the path. The order matters.
   `content-length` is a promise, and it used to be made from a stat of a NAME
   and then kept from a second look at the same name -- so a file renamed in
   between left the client holding `200 OK, content-length: N` and then a
   closed socket with nothing in it. That is worse than the 404 it would have
   got a microsecond earlier, because a client cannot tell a truncated body
   from a network fault. This application opens that window itself: the
   downloader renames `map.pmtiles` into place under this very root, and this
   endpoint serves it.

   An fd pins the inode it opened, the same fact `open_tile_archive` relies
   on, so once the handle is held nothing done to the name can change the
   bytes promised.

   The handle belongs to the server switch and is closed on every path below:
   at once on a 304 and on the not-a-file path, and under [Fun.protect] in the
   body callback otherwise, so a client that vanishes mid-stream still
   releases it. One case does not release it: cohttp writes the response
   header before calling the body, and if that write raises, the body never
   runs and the handle lives until the server stops. Fixing it would need a
   per-connection switch, which this callback is not given. *)
let serve_file cfg ~sw ~what ~root ~segments ~meth ~range_header ~if_none_match
  ...
  (* `Url_path.resolve` lets a 300-byte segment through, and is right to: it
     holds no separator and cannot leave the root, which is all
     `theorem_no_escape` claims and all that is needed. Whether a filesystem
     accepts a name that long is a fact about the directory, and F* is never
     told about directories. *)
  let opened =
  ...
  (* The kind is checked on the HANDLE, not on the path. A directory opens
     fine and fails only on the first read, and asking the name again would
     re-ask a question the handle has answered, about a name that may no
     longer mean the same file. *)
  let probed =
    match opened with
    | None -> None
    | Some file -> (
  ...
  | Some (file, stat) ->
      ...
      (* Size and mtime rather than a hash of the contents. These are the
         glyphs and sprites, and also map.pmtiles, a gigabyte served in
         ranges: reading a file end to end to decide whether to send it would
      ...
      in
      (* A Range against a validator that has moved on gets the whole file,
         not a window into different bytes. This endpoint serves map.pmtiles,
         which the downloader rewrites in place, so a client splicing a stale
         206 into a partial copy would be joining two archives. *)
      let range =
...
(* The arithmetic behind every answer the HTTP API gives: the C emitted by
   KaRaMeL from the F* proofs, reached over the FFI (ocaml/c_core). It
   replaced the extracted-OCaml core here after the two ran side by side long
   enough; that comparison still runs on every `make test`, but only one of
   them serves.

   Scope: `/api/*` is off unless the server is started with --api, and no UI
   path uses it. So in the default and desktop configurations this core
   computes nothing, and every address a user sees comes from the
   js_of_ocaml core in the browser. *)
let core = Tessarium_c_core.C_core.core

(* ------------------------------------------------------------- session API *)

(* Off by default, and the README says which mode is running. Enabling it
   means a seed phrase crosses the network, which is the thing client-side
   derivation exists to avoid. It stays because scripting and headless use
   need it, not because the UI does. *)
module Sessions = struct
  (* Derived keys expire. Without a TTL a long-running instance piles them up
     until it exits, turning a convenience cache into a growing heap of key
     material in memory. *)
  let ttl = 3600.0

  (* And a ceiling, because a TTL alone still allows unbounded growth inside
  ...
  (* Swept on write rather than on a timer: there is nothing to do when
...
(* The endpoints that spend or reveal key material share one token bucket.
   ...
   because each answer is a chosen plaintext/ciphertext pair and the oracle
   arithmetic in docs/fe1-security.md depends on this ceiling -- unthrottled,
   an encode oracle would answer a million queries in about a minute. The
   basemap endpoints stay outside: they touch tiles, never the key, and a
   download's status is polled every second. *)
          ...
          with_key (fun key ->
              ...
              | Some address -> (
                  ...
                  (* The C core refuses arguments outside its proved domain
                     ...
                     today; it is here so that if it ever becomes reachable
                     the answer is a 400 and not a dropped connection. *)
                  | exception Invalid_argument e -> bad e
...
(* Bounding boxes are plain JSON numbers. Floats are fine here: this is the
   tile-picking side, which never touches an address. *)
...
(* A polygon arrives as rings of [lon,lat] pairs. Anything malformed fails the
   whole region; the size caps are in the validator. *)
...
let parse_region json =
  ...
  | Some min_lon, Some min_lat, Some max_lon, Some max_lat, Some max_zoom -> (
      ...
      | Error e -> Error e
      | Ok polygon -> (
          (* What the picker called this place, so the progress view can name
             its bars. It arrives ON the region rather than in a separate
             array, so there is no second list to fall out of step with, no
             alignment check here and no silent blanking downstream. The
...
(* One request names one or more regions: the picker lets several countries,
   states and cities ride in one download, merged and deduplicated on the
   server. The cap is a sanity bound far above anything the picker sends. *)
...
(* The ledger id is a hex digest prefix this server handed out, so the only
   thing to defend against is garbage. *)
let parse_id json =
...
(* Which kind of failure a coverage query hit. Kept out of the handler
   because a Cohttp response cannot be read back: a viewport larger than one
   query allows is the caller asking for too much, while an archive this
   server cannot read is this server's own data gone wrong. Answering the
   second with a 400 blamed the page for it. *)
...
let handle_basemap cfg (ops : Basemap_download.ops) (settings : Settings.ops)
  ...
  (* A ledger or settings failure is this server's own data gone wrong, not
     the caller's request and not the tile source. *)
  ...
  | "basemap-export-delete" ->
      with_json body (fun json ->
          (* A file name, not a path. [Url_path] guards the route that SERVES
             these; this one deletes, so the name is checked against the
             directory listing downstream rather than matched here. *)
  ...
  | "basemap-browse" ->
      (* Gated on the opt-in setting here, not just in the UI: the page must
         not be able to make this server fetch from the network when the user
         has said no. *)
      if not (settings.browse_enabled ()) then
        ...
        with_json body (fun json ->
            let num name =
              ...
              | _ -> None
            in
            (* 15.0 means 15. JSON has one number type, and a serializer
               choosing a decimal spelling is not asking for a fractional
               zoom. Real fractions are still refused. *)
            let zoom =
                ...
                | Ok req -> (
                    ...
                    | Ok (fetched, written_zoom) ->
                        (* The zoom actually written, which the source's own
                           ...
                           reach keeps asking for tiles that never come. *)
                        respond_json cfg ~status:`OK
                          (`Assoc
                             [
                               ("ok", `Bool true);
  ...
  | "basemap-coverage" ->
      (* Where the map goes blank. Reads the archives on disk and nothing
         else: no network, no setting to gate, no key material. So it answers
         while a download runs and while browsing is off. *)
      with_json body (fun json ->
          let num name =
            match json_field name json with
            | Some (`Int i) -> Some (float_of_int i)
          ...
          (* Integer-valued floats are accepted for the same reason browse
             accepts them: JSON has one number type, and a serializer that
             writes 12.0 is not asking for a fractional zoom. *)
          let zoom =
            match json_field "zoom" json with
            | Some (`Int z) -> Some z
            | Some (`Float f) when Float.is_integer f && Float.abs f <= 32768.
          ...
          | _ ->
              bad "expected min_lon, min_lat, max_lon, max_lat and zoom 0..15")
  | "basemap-search" ->
      (* Names from the regions already on disk. No network, by design: a
  ...
  | "basemap-settings" ->
      with_json body (fun json ->
          ...
          | days, browse -> (
              ...
              | Ok payload ->
                  (* Off means gone. The browse cache records where the user
                     looked, so turning the setting off erases it rather than
                     leaving it dormant and still serving. Returns at once
                     whether or not a writer holds the file: one that does
                     erases it as it finishes, and browsing is already off,
                     so nothing new can arrive meanwhile. *)
      ...
      | json -> (
          ...
          | Ok reqs -> (
              ...
              if String.equal endpoint "basemap-estimate" then
                ...
                | Ok payload -> respond_json cfg ~status:`OK payload
                | Error e ->
                    (* The failure is upstream: the tile source is
                       unreachable or broken. The status code should say so
                       rather than blame the request. *)
                    error cfg ~status:`Bad_gateway e
              else
                (* The world overview keeps no ledger entry, so a name sent
                   with one has nowhere to go. Ignored rather than refused,
                   because the client sends the same shape either way. *)
                match json_field "name" json with
...
(* Whether a request declared a body. Both mistakes here are real and both
   were made: reading a body that was never declared waits on a keep-alive
   connection for bytes that never come, and NOT draining one that was
   declared leaves its bytes to be parsed as the start of the next request,
   which fails every later request on that connection. Only the headers can
   answer this. *)
...
(* Taking the body off the socket is the only part of the /api/ guard that
   touches Eio, so it is the only part that lives here. Everything the guard
   DECIDES is in Api_guard, behind an abstract type the handlers demand. *)
let read_body flow =
...
(* No wildcard, so a new refusal has to be given a status and a sentence here
...
let handler cfg ~sw ~ui_root ~basemap_root ~sessions ~limiter ~clock ~random
  ...
  fun _conn (request : Http.Request.t) body ->
    ...
    let result, status, bytes, range =
      ...
      (* One branch for every /api/ route. The checks are no longer branches
         above this one; they are in the type [handle_api] and
      ...
      (* The one route whose body is not read into memory. It carries a map
         archive of several gigabytes, streamed to disk as it arrives.
         ...
         the whole reason this is not an /api/ endpoint. *)
      | Route.Import -> (
          match
      ...
      | Route.Asset segments -> (
          ...
          (* Embedded first, then the directory: when the binary carries a
             UI, that UI is what ships, and --ui only fills the gap in a
             binary built without one. The trap: after `npm run build`, a
             stale ocaml/server/ui_dist keeps serving the OLD interface
             whatever --ui says. Refresh it (make ui) and rebuild. *)
          let from_disk () =
            let served =
            ...
            (* A single-page app owns its own routes: /somewhere returns the
               shell so a deep link survives a reload. A missing .js does
               not. *)
...
let run env ~sw ~port cfg =
  ...
  (* Loopback only. This binary is the desktop app as well as the self-hosted
     server, and a desktop app listening on every interface is a mistake that
