  ...
  Js.export "tessarium"
    object%js
      ...
      (* The browser derives keys with the Argon2id wasm module, not with
          ...
          on success, else the reason (an invalid phrase) ready to
          display. *)
      val derivationVersion = jstr Tessarium.derivation_version

      method kdfInputs mnemonic =
        ...
        | Ok () ->
            inputs
              (hex_of_string (Tessarium.kdf_password ~mnemonic:(ostr mnemonic)))
              (hex_of_string Tessarium.kdf_salt)
              Js.Opt.empty
      ...
      method encodeDeg keyHex lat lon =
        let key = string_of_hex (ostr keyHex) in
        jstr
          (Tessarium.encode_z ~core ~key ~lat:(ns_of_deg lat)
             ~lon:(ns_of_deg lon))

      method decodeDeg keyHex addr =
        let key = string_of_hex (ostr keyHex) in
        match
          core.Tessarium.decode ~key (Tessarium.address_of_string (ostr addr))
        with
        | None -> Js.null
        | Some (lat, lon) ->
            Js.some
      ...
      method cellBoundsDeg lat lon =
        let a, b, c, d =
          Tessarium.cell_bounds_z ~core ~lat:(ns_of_deg lat)
            ~lon:(ns_of_deg lon)
        in
        object%js
      ...
      method gridForBounds latLo lonLo latHi lonHi limit =
        ...
        List.iteri
          (fun i (a, b, c, d) ->
            let put k v =
              Typed_array.set flat ((i * 4) + k) (Js.number_of_float v)
            in
            put 0 (deg_of_ns a);
            put 1 (deg_of_ns b);
            put 2 (deg_of_ns c);
