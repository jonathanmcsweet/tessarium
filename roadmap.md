...
- [ ] **The giants still cannot go full depth in one download.** France
      (9.4 GB, 2.3M tiles) and every US state now download at street level;
      Brazil, Russia, whole US/Canada, and Alaska (its box crosses the
      antimeridian and comes back world-wide) exceed the 6,000,000-id
      planning ceiling and fall back to regional detail with the card saying
      to pick a state. Going bigger needs chunked, resumable downloads — an
      hours-long multi-gigabyte fetch that dies at 90% must not start over,
      and losing the whole .part on cancel is the same problem. Two smaller
      debts from the same work: the final directory build of a country-sized
      archive blocks the server for a few seconds at the end of the write,
      and a full-country plan holds roughly 300–400 MB transiently. The
      multi-select picker compounds this: a selection's regions plan
      sequentially in one request, so several full-depth countries at once
      multiply the ~15 s and the transient memory, and enough of them can
      outlive the client's 120 s estimate timeout.
- [ ] **Region boxes are boxes.** The picker (Natural Earth data,
      committed catalogue, regenerated by tools/gen-regions.py) uses
      bounding boxes, so a country download includes slivers of its
      neighbours; cities are points in the source data, so their boxes are
      drawn by the generator — sized by prominence and latitude, a
      judgement, not a boundary — and countries crossing the antimeridian — Russia, Fiji,
      the US via the Aleutians — get a near-world-wide box that the tile
      budget then flattens to overview depth. Polygon-clipped planning
      (tile-intersects-polygon instead of tile-in-box) and splitting wrapped
      boxes at 180° are the fixes; both belong together.
