...
- [ ] **Country downloads stop at regional detail.** The server plans at
      ...
      that budget France estimates at ~210 MB and zoom 10 — every town and
      main road, not every street. Street level for a whole country is a
      real want (the established offline apps ship it; France is gigabytes
      there) and needs three things the budget currently substitutes for:
      planning that yields to the scheduler instead of blocking it, a
      resumable download (an hour-long fetch that dies at 90% must not start
      over), and a progress story for archives in the gigabytes. Until then
      the card says honestly that a big area stops at regional detail.
