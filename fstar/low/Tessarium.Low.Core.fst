...
/// recomputes on random draws in the harness (see Low.Blake2s's header).
...
module B = Tessarium.Low.Blake2s
...
noextract
let rf_real : F.round_fn B.key8 unit =
  fun k _ i x m ->
...
let rf_real_ghost : G.erased (F.round_fn B.key8 unit) = G.hide rf_real
...
#push-options "--ifuel 1"
let rf_real_low (k: B.key8) (t: unit)
  ...
  let r3 = U64.rem (U64.add (U64.mul r2 0x100000000uL) h0) m in
  horner_step (U64.v h3) (U64.v r0) (U64.v h2) (U64.v m);
  horner_step (U64.v h3 * 0x100000000 + U64.v h2) (U64.v r1) (U64.v h1) (U64.v m);
  horner_step ((U64.v h3 * 0x100000000 + U64.v h2) * 0x100000000 + U64.v h1)
    (U64.v r2) (U64.v h0) (U64.v m);
  r3
#pop-options

(* ------------------------------------------------------- extraction roots *)

/// Key arrives as eight flat words (the KDF's 32-byte output, as
/// BLAKE2s's little-endian words) so the C ABI carries no struct layout.

let core_encrypt (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (x: U64.t{U64.v x < S.addr_space})
  : (y: U64.t{U64.v y < S.addr_space})
  = L.encrypt_low #B.key8 #unit #rf_real_ghost rf_real_low
      (k0, k1, k2, k3, k4, k5, k6, k7) () x

let core_decrypt (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (y: U64.t{U64.v y < S.addr_space})
  : (x: U64.t{U64.v x < S.addr_space})
  = L.decrypt_low #B.key8 #unit #rf_real_ghost rf_real_low
      (k0, k1, k2, k3, k4, k5, k6, k7) () y

let core_encode (cum: LG.cum_low) (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (dlat: U64.t{U64.v dlat <= S.lat_span})
    (dlon: U64.t{U64.v dlon <= S.lon_span})
  : U64.t & U64.t & U64.t & U64.t
  = LA.encode_low #B.key8 #unit #rf_real_ghost cum rf_real_low
      (k0, k1, k2, k3, k4, k5, k6, k7) () dlat dlon

let core_decode (cum: LG.cum_low) (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (w1: U64.t{U64.v w1 < S.words}) (w2: U64.t{U64.v w2 < S.words})
    (w3: U64.t{U64.v w3 < S.words}) (n: U64.t{U64.v n < S.num_max})
  : U64.t & U64.t & U64.t
  = LA.decode_low #B.key8 #unit #rf_real_ghost cum rf_real_low
...
/// half (see Low.Blake2s's header), not part of this statement. No agreement
/// lemma is needed: the ghost index of the Feistel here IS rf_real, so
/// encode_low's ensures already says exactly this.
val theorem_core_encode (cum: LG.cum_low) (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (dlat: U64.t{U64.v dlat <= S.lat_span})
    (dlon: U64.t{U64.v dlon <= S.lon_span})
...
val theorem_core_decode (cum: LG.cum_low) (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (w1: U64.t{U64.v w1 < S.words}) (w2: U64.t{U64.v w2 < S.words})
    (w3: U64.t{U64.v w3 < S.words}) (n: U64.t{U64.v n < S.num_max})
...
val theorem_core_roundtrip (k0 k1 k2 k3 k4 k5 k6 k7: B.w32)
    (x: U64.t{U64.v x < S.addr_space})
...
let theorem_core_roundtrip k0 k1 k2 k3 k4 k5 k6 k7 x =
  L.theorem_roundtrip_low #B.key8 #unit #rf_real_ghost rf_real_low
    (k0, k1, k2, k3, k4, k5, k6, k7) () x
