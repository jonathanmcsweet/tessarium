...
def _fstar_list(name: str, values: list, ty: str, per_line: int) -> str:
    """An F* list literal, wrapped so the file stays readable in a diff."""
    lines = [f"let {name} : list {ty} = ["]
...
def emit_fstar(t: dict, path) -> None:
    """Write the band table as an F*-consumable constant module.

    Reads the committed table rather than regenerating it: this emitter must
    never be able to perturb the grid. Data only, no lemmas — the
    well-formedness proofs that consume this live in Tessarium.Grid, so that
    this file stays a table rather than an argument.
    """
    path.write_text(f'''module Tessarium.BandTable
...
/// Column count per band, taken from the cos of the band's equator-ward edge,
/// so no cell in a band is ever wider than the {t["target_m"]} m target. Cells get
/// narrower toward the pole within a band, never wider.
{_fstar_list("col_counts_list", t["col_counts"], "nat", 10)}

/// Cumulative cell offset at the start of each band:
///   offsets[b + 1] == offsets[b] + col_counts[b] * rows_per_band
/// Stored rather than recomputed because cell_to_point binary-searches it.
{_fstar_list("offsets_list", t["offsets"], "nat", 6)}
''')
...
if __name__ == "__main__":
    if "--emit-fstar" in sys.argv:
        ...
        out = HERE.parent / "fstar" / "Tessarium.BandTable.fst"
